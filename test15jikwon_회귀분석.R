#원격DB(MariaDB)와 연동하여 자료를 읽은후 jikwon 테이블로 근무년수의 따른 연봉 예측 모델 작성!!
...
head(jik_ibsail,3)
typeof(jik_ibsail)  #list type 
jik_ibsail <- as.numeric(unlist(jik_ibsail)) # list 였던 jik_ibsail 을 숫자로 바꾸기
typeof(jik_ibsail)
...
jik_data <- data.frame(jik_workyear,jik_pay)  # dataframe 으로 만들어 놓기 ! 
head(jik_data)
...
summary(model)   #6.943e-13 < 0.05 유의한 모델, R-squared:  0.8458 독립변수가 종속변수의 분산을 84.5% 정도의 설명을 하고 있다
